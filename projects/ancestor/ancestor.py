...
def earliest_ancestor(ancestors, starting_node):
    ...
    while neighbors_to_visit.size() > 0:
        ...
        if (current_vertex not in visited_neighbors):
            ...
            # also, if it's the same length as the longest one we have seen,
            # if the ancestor has smaller ID, use it
            if ((len(current_path) >= max_path_len and 
            current_vertex < earliest_ancestor) or 
            (len(current_path) > max_path_len)):
